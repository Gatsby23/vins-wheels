...
int main(int argc, char** argv)
{
	...
	// load image list
	FILE* file;
	file = std::fopen((dataPath + "image_name.txt").c_str() , "r");
	if(file == NULL){
	    printf("cannot find file: %image_name.txt\n", dataPath.c_str());
	    ROS_BREAK();
	    return 0;          
	}
	string imageName;
	vector<std::string> imageNameList;
	ifstream stmFile(dataPath + "image_name.txt");
	while ( stmFile>>imageName)
	{
	...
	for (size_t i = 0; i < imageNameList.size(); i++)
	{
		if(ros::ok())
		{
		    string sImageTime=imageNameList[i].substr(0,16);
		    stringstream ssImageTime;
            ssImageTime<<sImageTime;
		    double imageTime=0;
            ssImageTime>>imageTime;
			printf("\nprocess image %d\n", (int)i);
			stringstream ss;
			ss << setfill('0') << setw(10) << i;
			leftImagePath = dataPath + imageNameList[i];
			rightImagePath = dataPath + imageNameList[i];
			//printf("%lu  %f \n", i, imageTimeList[i]);
			//printf("%s\n", leftImagePath.c_str() );
			...
			imRightMsg->header.stamp = ros::Time(imageTime);
			pubRightImage.publish(imRightMsg);


			estimator.inputImage(imageTime, imLeft);//时间戳 左目 右目
